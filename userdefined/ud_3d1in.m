function [DEFL,REACT,ELE_FOR,ELE_YLD,AFLAG,APRATIOS,LIMIT_STATE] = ud_3d1in(...
		nnodes,coord,concen,fixity,nele,ends,A,Izz,Iyy,J,Cw,IsSym,Ysc,Zsc,Betay,Betaz,Betaw,Zzz,Zyy,Ayy,Azz,...
		E,v,Fy,YldSurf,Wt,webdir,beta_ang,w,thermal,truss,anatype,sol_scheme,numsteps,...
		ratio_req,stop_ratio,restart,defl,react,ele_for,ele_yld,ele_pldef,...
		apratios,limit_state,h_stat_mes)
%UD_3D1IN performs a user defined three-dimensional
% first-order inelastic analysis of a structural system.
%
...
%                              sol_scheme = 2   Other 1
%                              sol_scheme = 3   Other 2
%                              sol_scheme = 4   Other 3
...
fid = fopen(fname,'w');
fprintf(fid,'*Node, Type = NFrame2D \n');

for i = 1 : nnodes                    

    fprintf(fid,'%6i, %8.2f, %8.2f\n',i,coord(i,1),coord(i,2));
...
%% Distribution Load Information

fprintf(fid,'\n');

fprintf(fid,'\n*MLOAD, TYPE = DIST, Amplitude = gravity\n');
...
for i = 1 : nele
    ...
    fprintf(fid,'%4i,\b\b%2d,\b\b%2f\n',i,j,R);
     
    end
    
end

%% Concentrated Load Information

fprintf(fid,'\n');

fprintf(fid,'\n*NLOAD, AMPLITUDE = Transload\n');
...
for i = 1 : nnodes
    ...
    fprintf(fid,'%4i,\b\b%2d,\b\b%4.2d\n',i,j,R);
...
fprintf(fid,'\n*Steel, Name = A36, Type = Cyclic\n');

for i = nele - 1
    
    if Fy(i,1) == Fy(i+1) && E(i,1) == E(i+1,1) && v(i,1) == v(i+1)
        
        fprintf(fid,'%4.2d,\b%4.2d,\b%4.2f\n',Fy(i,1),E(i,1),v(i,1));
        
    elseif Fy(i,1) ~= Fy(i+1) || E(i,1) ~= E(i+1,1) && v(i,1) ~= v(i+1)
       
        fprintf(fid,'%4.2d,\b%4.2d,\b%4.2f\n',Fy(i+1,1),E(i+1,1),v(i+1,1));
