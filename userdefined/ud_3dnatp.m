function [DEFL,REACT,ELE_FOR,AFLAG,PERIODS] = ud_3dnatp(...
		nnodes,coord,concen,fixity,nele,ends,A,Izz,Iyy,J,Cw,IsSym,Ysc,Zsc,Betay,Betaz,Betaw,Zzz,Zyy,Ayy,Azz,...
		E,v,Fy,YldSurf,Wt,webdir,beta_ang,w,thermal,truss,req_modes,h_stat_mes)
%UD_3DNATP performs a user defined three-dimensional elastic
% eigenvalue analysis to determine the natural modes of
% vibration for a given structural system.
%
...
%                                      text_mess = ['Performing step #',istep];
...
%
%     Output Information:
%       DEFL(i,1:6,m)    ==  node i's calculated 6 d.o.f. deflections for all m modes
%                            of the analysis.  For a supported d.o.f., the proper
%                            percent of the prescribed value, fixity(i,dof), should be
%                            inserted in DEFL.
%                            Example: DEFL(12,3,5) is node 12's Z-disp. component
%                                     for the fifth buckling mode.
...
%                              DEFL(i,1,m) = displacement in X direction at end of mode m
%                              DEFL(i,2,m) = displacement in Y direction at end of mode m
%                              DEFL(i,3,m) = displacement in Z direction at end of mode m
%                              DEFL(i,4,m) = rotation about X direction at end of mode m
%                              DEFL(i,5,m) = rotation about Y direction at end of mode m
%                              DEFL(i,6,m) = rotation about Z direction at end of mode m
%       REACT(i,1:6,m)   ==  node i's calculated 6 d.o.f. reactions for all m modes
%                            of the analysis.  For free d.o.f., a value of 0.0 should be
%                            inserted in REACT. 
...
%                              REACT(i,1,m) = force in X direction at end of mode m
%                              REACT(i,2,m) = force in Y direction at end of mode m
%                              REACT(i,3,m) = force in Z direction at end of mode m
%                              REACT(i,4,m) = moment about X direction at end of mode m
%                              REACT(i,5,m) = moment about Y direction at end of mode m
%                              REACT(i,6,m) = moment about Z direction at end of mode m
%       ELE_FOR(i,1:1?,m)==  element i's calculated element end forces for all m modes
%                            of the analysis.
%                            Note: All values reference the element's local
%                                  coordinate system.
%                            Example: ELE_FOR(17,9,5) is element 17's Z-shear
%                                     force at the end node of the element for the fifth
%                                     mode.
%                              ELE_FOR(i,1,m)  = x-force at start node for mode m
%                              ELE_FOR(i,2,m)  = y-force at start node for mode m
%                              ELE_FOR(i,3,m)  = z-force at start node for mode m
%                              ELE_FOR(i,4,m)  = x-moment at start node for mode m
%                              ELE_FOR(i,5,m)  = y-moment at start node for mode m
%                              ELE_FOR(i,6,m)  = z-moment at start node for mode m
%                              ELE_FOR(i,7,m)  = x-force at end node for mode m
%                              ELE_FOR(i,8,m)  = y-force at end node for mode m
%                              ELE_FOR(i,9,m)  = z-force at end node for mode m
%                              ELE_FOR(i,10,m) = x-moment at end node for mode m
%                              ELE_FOR(i,11,m) = y-moment at end node for mode m
%                              ELE_FOR(i,12,m) = z-moment at end node for mode m
...
%                              ELE_FOR(i,13,m) = bimoment at start node for mode m
%                              ELE_FOR(i,14,m) = bimoment at end node for mode m
%                              ELE_FOR(i,15,m) = rate of twist at start node for mode m
%                              ELE_FOR(i,16,m) = rate of twist at end node for mode m
...
%       PERIODS(i,1)    ==  recipricol of eigenvalue of each mode.  Modes should be sorted
%                            according to descending periods.
%                            Note that PERIODS is a COLUMN vector.
%                            size(PERIODS,1) = total number of modes found.
%
